...
- 自由格式
    - `!`后面的文本都是注释
    - 每行可以编写132个字符
    - 一行代码最后接符号`&`表明接续下一行，如果开头有`&`表明接续上一行
### 数据类型
- 整数
    - 长整型
        - 声明`integer(kind=4) a`
...
- 复数
    - 由两个浮点数来表示`complex`
- 字符
    - 声明需要给定最大长度`character(kind=10) a`
    - `string(a:b)="...."`可以重置第a个字符到第b个字符
    - 字符串可以通过`//`连接
    - 常用函数
...
- 以固定宽度输出对应数据类型的数
    ...
    - 浮点数`Fw[.b]` w宽度，小数保留b位。科学计数法`Ew.b[Ec]` w宽度，小数占b位，指数占c位
    - 字符串`Aw` w宽度
    - 逻辑值`Lw` w宽度
    - 使用G可以输出所有类型的数据
...
- `implicit none`接在`program`命令的下一行，关闭默认类型，所有变量都需要声明
...
#### 控制命令
`cycle`相当于c中的`continue`，`exit`命令退出当前正在执行的循环（注意如果`exit`在嵌套中，退出的是最内层的）
